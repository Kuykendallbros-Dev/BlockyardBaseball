...
/**
 * Ball position at flight fraction `u` (0 = release, 1 = plate). Values outside
 * [0, 1] are clamped, so callers can pass raw elapsed/duration.
 */
export function ballPositionAt(u: number): [number, number, number] {
  const t = u < 0 ? 0 : u > 1 ? 1 : u;
  const [x0, y0, z0] = RELEASE_POINT;
  const [x1, y1, z1] = PLATE_POINT;

  const x = x0 + (x1 - x0) * t;
  const z = z0 + (z1 - z0) * t;
  // straight-line height plus a parabola that is 0 at the ends, max at t = 0.5
  const y = y0 + (y1 - y0) * t + ARC_HEIGHT * 4 * t * (1 - t);
